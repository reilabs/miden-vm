...
fn test() {
    ...
    //let stack: Vec<_> = (500..(1<<16)).into_iter().collect();
    //let operations: Vec<_> = (500..(1<<16)).flat_map(|_| vec![Operation::U32split, Operation::U32add, Operation::U32xor]).collect();
    let stack = [65535, 1564894994948];
    let operations = vec![Operation::U32split, Operation::U32add];
    ...
    //      being the Lagrange kernel and the second being  (\sum_{j=0}^l mls[j][i] * \lambda_i)_{i\in\{0,..,n\}}
    ...
    for k in 1..=nu {
        ...
        let divisor_k_1: Vec<Felt> = (0..trace_len)
            .into_iter()
