...
use winter_prover::{crypto::{Digest, MerkleTree}, matrix::{ColMatrix, RowMatrix}, proof::Queries, CompositionPoly, CompositionPolyTrace, ConstraintCommitment, ConstraintCompositionCoefficients, DefaultConstraintEvaluator, EvaluationFrame, Prover, StarkDomain, TraceInfo, TraceLde, TracePolyTable, TraceLayout, AuxTraceRandElements};
...
>(
    ...
    // allocate space to store all the results
    let mut lde_out = alloc_pinned(lde_domain_size * num_base_columns * size_of::<E::BaseField>(), E::ZERO);
    let mut lde_out_transposed = alloc_pinned(lde_domain_size * num_base_columns * size_of::<E::BaseField>(), E::ZERO);
    let mut hash_states_out = alloc_pinned(partition_count * CAPACITY * lde_domain_size * size_of::<E::BaseField>(), D::default());
    let mut merkle_tree_out = alloc_pinned((2 * lde_domain_size - 1) * CAPACITY * size_of::<E::BaseField>(), D::default());
