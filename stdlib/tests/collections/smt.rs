...
const LEAVES_MULTI: [(RpoDigest, Word); 2] = [
    (RpoDigest::new(word(101, 102, 103, 0x01a4)), word(0x1, 0x2, 0x3, 0x4)),
    // Most significant felt does NOT differ from previous.
    (RpoDigest::new(word(201, 202, 203, 0x01a4)), word(0xb, 0xc, 0xd, 0xe)),
];
...
fn test_smt_get_multi() {
    let expect_value_from_get = |key: RpoDigest, value: Word, smt: &Smt| {
        let source = "
            use.std::collections::smt
